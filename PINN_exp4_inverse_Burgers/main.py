...
'''
PINN实现偏微分方程的反问题，即利用边界条件，偏微分方程，定义域，来求解输出的分布，以及对应的参数
'''
import torch
...
def interior(n=N):
    ...
    # 这个也是和其他的深度学习模型不同的，其他的一般只需要对训练的参数进行梯度设置，
    # 但是因为偏微分方程中，需要对这些输入求解一阶导，甚至二阶导数，因此外面需要额外的对这些参数进行设置。
...
# 初始化一个需要训练的参数
v= torch.nn.Parameter(torch.tensor(0.01), requires_grad=True)
def l_interior(u):
    # 损失函数L1
    x, t, cond = interior()
...
opt = torch.optim.Adam(params=list(u.parameters()) + [v]) # 定义优化器

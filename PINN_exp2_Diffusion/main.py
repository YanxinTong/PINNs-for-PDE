...
# Domain and Sampling，内点采样，这里的Diffusion的输入是x和t，输出的是u
def interior(n=N):
...
def data_interior(n=N2):
    ...
    cond = torch.sin(torch.pi*x)*torch.exp(-t)
    return x.requires_grad_(True), t.requires_grad_(True), cond
...
def l_interior(u):
    ...
    return loss(gradients(uxt, t, 1) - gradients(uxt, x, 2), cond)
...
ax = Axes3D(fig)
fig.add_axes(ax)
ax.plot_surface(xm.cpu().detach().numpy(), tm.cpu().detach().numpy(), u_real_fig.cpu().detach().numpy())
